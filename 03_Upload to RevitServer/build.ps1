...
rem чем те же модели по одной. Список — models.пример.txt рядом.
chcp 65001 >nul
cd /d "%~dp0.."

bin\RvsUpload.exe ^
  --list "%~dp0models.пример.txt" ^
...
foreach ($имя in $примеры.Keys) {
    # UTF-8 с BOM: без него cmd.exe ломает кириллицу в комментариях.
    [IO.File]::WriteAllText("$dist\Примеры\$имя", $примеры[$имя], (New-Object Text.UTF8Encoding $true))
}
...
# чем те же модели по одной. Список — models.пример.txt рядом.
$ErrorActionPreference = "Stop"
$exe = Join-Path $PSScriptRoot "..\bin\RvsUpload.exe"

& $exe --list (Join-Path $PSScriptRoot "models.пример.txt") `
       --dest-folder "RSN://СЕРВЕР/Проекты/2026" `
       --revit-version 2024 `
       --overwrite `
       --create-folders `
...
foreach ($имя in $примерыPs.Keys) {
    ...
    [IO.File]::WriteAllText("$dist\Примеры\$имя", $примерыPs[$имя], (New-Object Text.UTF8Encoding $true))
}
...
'@
[IO.File]::WriteAllText("$dist\Примеры\models.пример.txt", $список, (New-Object Text.UTF8Encoding $true))
