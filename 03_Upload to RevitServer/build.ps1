...
<#
.SYNOPSIS
    Записывает пакетный файл так, чтобы интерпретатор его выполнил.
#>
function Написать-Cmd {
    param([string]$Путь, [string]$Текст)
    ...
    # перескакивает середину строки: в файле длиннее 512 байт из
    # комментария вываливался обрывок и выполнялся как команда.
    $Текст = $Текст -replace "`r?`n", "`r`n"

    # В CP866 нет длинного тире и кавычек-ёлочек: молча превратились бы в «?».
    # Лучше уронить сборку, чем отдать пользователю файл с мусором.
    if ($oem.GetString($oem.GetBytes($Текст)) -ne $Текст) {
        throw "Файл «$(Split-Path $Путь -Leaf)» содержит символы, которых нет в CP866. Замените длинное тире на дефис, ёлочки на обычные кавычки."
    }

    [IO.File]::WriteAllBytes($Путь, $oem.GetBytes($Текст))
}

Copy-Item "tools\Установить.ps1" $dist -Force

# Запуск двойным щелчком. Правой кнопкой по .ps1 -> «Выполнить с помощью
# PowerShell» окно закрывает сразу, и человек не видит ни результата,
# ни ошибки. Этот файл держит окно открытым сам.
#
# Имя скрипта не пишем буквами: в нём кириллица, а строка команды читается
# в кодировке окна консоли. Берём единственный .ps1 рядом — тогда в самом
# пакетном файле остаётся только латиница, а имя приходит из файловой
# системы уже готовым.
$запускУстановки = @'
@echo off
rem Ставит надстройку RvsUpload во все найденные версии Revit.
rem Запускать двойным щелчком: окно останется открытым и после ошибки.
cd /d "%~dp0"

set "SCRIPT="
for %%F in ("*.ps1") do set "SCRIPT=%%~fF"

if not defined SCRIPT (
  echo Рядом с этим файлом нет скрипта установки.
  echo Похоже, поставка распакована не полностью.
  echo.
  pause
  exit /b 1
)
...
echo.
if not "%CODE%"=="0" echo Установка завершилась с ошибкой, код %CODE%. Текст ошибки выше.
pause
exit /b %CODE%
'@
Copy-Item "Settings.html" "$dist\Настройки.html" -Force
Написать-Cmd -Путь "$dist\Установить.cmd" -Текст $запускУстановки
Write-Host "  Установить.ps1 и Установить.cmd"
...
foreach ($имя in $примеры.Keys) {
    Написать-Cmd -Путь "$dist\Примеры\$имя" -Текст $примеры[$имя]
}
...
С ЧЕГО НАЧАТЬ
  1. Двойной щелчок по «Установить.cmd». Окно останется открытым, пока
     вы его не закроете, — результат и ошибки будут видны.
...
ЧТО ЗДЕСЬ ЛЕЖИТ
  Установить.cmd     ставит надстройку во все найденные Revit
  Установить.ps1     то же самое для тех, кто работает в консоли
