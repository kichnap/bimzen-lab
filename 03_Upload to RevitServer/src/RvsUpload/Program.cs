...
namespace RvsUpload
{
    internal static class Program
    {
        ...
        private static int Run(string[] rawArgs)
        {
            ...
            // Без повторов остаток пакета терялся.
            ...
            while (true)
            {
                ...
                // На повторе открываем с проверкой структуры. Повторяем мы ровно
                // те модели, с которыми уже что-то не так, а Audit чинит мелкие
                // повреждения — из-за них Revit и падает при открытии через раз.
                // На первой попытке он не нужен: замедлил бы открытие всего пакета
                // ради тех единиц, что упадут.
                if (attempt > 1 && opt.AuditOnRetry)
                {
                    var впервые = pending.Where(t => !t.Audit).ToList();
                    foreach (var t in впервые) t.Audit = true;

                    if (впервые.Count > 0)
                        Log($"Повтор: {впервые.Count} из {pending.Count} моделей открываются " +
                            "с проверкой структуры (audit) — медленнее, но чинит мелкие повреждения. " +
                            "Отключается ключом --no-audit-on-retry.");
                }
                                        ...
                                        Dictionary<string, ServerProperties> limits)
        {
            ...
            if (!clients.TryGetValue(host, out var client))
            {
                ...
                // это через пять-шесть минут после запуска Revit.
...
ПАРАМЕТРЫ
  ...
  --no-audit-on-retry      Не включать проверку структуры модели на повторных
                           попытках. По умолчанию на повторе модели открываются
                           с проверкой: повторяются те, с которыми уже что-то
                           не так, а проверка чинит мелкие повреждения. Открытие
                           при этом заметно медленнее
