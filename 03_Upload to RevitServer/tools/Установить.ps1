...
param(
    [int[]]$RevitVersions = @(),
    [switch]$Удалить,
    # Не ждать нажатия Enter в конце. Нужно, если установка вызывается
    # из другого скрипта: там ждать ответа некому.
    [switch]$БезПаузы
)
...
# Запущен ли скрипт из проводника — правой кнопкой «Выполнить с помощью
# PowerShell». В этом случае окно закрывается сразу после последней строки,
# и человек не успевает прочитать ни результат, ни ошибку. Определяем
# по родительскому процессу: у запуска из проводника это explorer.
$ЗапущенИзПроводника = $false
try {
    $родитель = (Get-CimInstance Win32_Process -Filter "ProcessId = $PID" -ErrorAction Stop).ParentProcessId
    $ЗапущенИзПроводника = ((Get-Process -Id $родитель -ErrorAction Stop).ProcessName -eq "explorer")
} catch {
    # Определить не вышло — не беда: паузы просто не будет
}

<#
.SYNOPSIS
    Завершает работу, задержав окно, если его закрытие съело бы вывод.
#>
function Финиш([int]$Код = 0) {
    if ($ЗапущенИзПроводника -and -not $БезПаузы) {
        Write-Host ""
        Write-Host " Окно останется открытым. Нажмите Enter, чтобы закрыть." -ForegroundColor DarkGray
        [void](Read-Host)
    }
    exit $Код
}
...
if ($ошибки) { Строка "`nУстановка прошла не полностью: ошибок $ошибки." Red; Финиш 1 }
