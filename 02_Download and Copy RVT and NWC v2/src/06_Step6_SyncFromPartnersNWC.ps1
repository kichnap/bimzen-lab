...
$NamePatterns = Get-CleanPatternList -Patterns $config.ModelsNWCFromPartners
...
if ($NamePatterns.Count -eq 0) {
	Write-Log -Message ($taskName + ": не задан список моделей смежников (ModelsNWCFromPartners), копирование пропущено") -LogFile $logPath -Level WARN
	Write-LogDivider -LogFile $logPath -Title "Конец работы $taskName"
	return
}
